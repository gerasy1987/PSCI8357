...
simulate <- function(rel, sims = 100) {
  replicate(
    n = 1000,
    {
      ...
      c(
        rel = rel,
        HC2 = unname(tau >= naive_model$conf.low[2] & tau <= naive_model$conf.high[2]),
        CR2 = unname(tau >= robust_se$conf.low[2] & tau <= robust_se$conf.high[2])
      )
    }
  ) |>
    apply(MARGIN = 1, mean)
...
p <- 
  ...
  geom_hline(aes(yintercept = rel), linetype = "dashed", alpha = .5, size = .25) +
...
gganimate::animate(p, 
  ...
  fps = 15, duration = 10, rewind = TRUE)
