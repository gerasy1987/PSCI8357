...
- Your assignment should be submitted as a PDF file compiled from \LaTeX, [R]{.proglang} Markdown, or (_ideally_) a Quarto Markdown document.
    - If using raw \LaTeX (.tex) for your answers, submit an accompanying [R]{.proglang} file for any computational tasks, with referenced line numbers corresponding to each specific task.
...
The final exam will be held **in person** during the final examination week. The exam is 90 minutes long and will be administered as a Quarto Markdown template that you complete on your laptop and submit as a compiled PDF via Brightspace.

The exam is **closed-book**, with one exception: you may bring a **reference sheet** (1-2 standard letter-size page, front and back). The sheet may be handwritten or printed; if printed, the font size must be no smaller than 8pt and the page must have reasonable margins. No other notes or electronic materials are permitted.

The exam covers material from all lectures and consists of three parts:

-   **Conceptual questions** — multiple choice and true/false with brief justifications
-   **Short answer and derivations** — written responses involving identification arguments, light algebra, and research design reasoning
-   **Applied scenarios** — interpretation of provided R output (regression tables, diagnostic plots) from realistic research settings. Some questions may ask you to describe an approach for implementing a statistical procedure in [R]{.proglang} or to interpret provided [R]{.proglang} code.

The exam aims to assess individual progress and understanding of identification strategies, assumptions, and applied reasoning, thus allowing me to provide personalized recommendations for improving your methodological foundations. If you cannot take the exam during the scheduled period, you must provide notice at least one week in advance so that we can arrange an alternative time.
...
***Note:** The schedule is subject to change. Readings will be distributed via Brightspace ahead of each class.*

:::{.list-table aligns=c,l,l,l widths=1,2,4,4 header-cols=1}
...
*   - **1/12**
    ...
    - observation versus intervention, potential outcomes (ATE, ATT, probability of necessity)
...
*   - **1/21**
    ...
    - causal estimands, SUTVA, selection bias, causal diagrams (DAGs) 
    -   - @gerber2012field [Ch. 2]
        - @angrist2009mostly [Ch. 1-2]
        - @morgan2015counterfactuals [Ch. 1-2]
        - @holland1986statistics
...
*   - **1/26 1/28**
    - No Class
    - Winter Storm, **problem set 1 is posted on 1/26**
    -

*   - **2/2 2/4**
    - Regression and Causality
    - CEF and its properties, selection on observables, conditional ignorability, positivity assumption
...
*   - **2/9**
    ...
    - regression anatomy, omitted variable bias, Back-Door Criterion and bad controls
...
*   - **2/11 2/16**
    ...
    - target quantities (SATE, PATE), inference for the SATE and PATE in an idealized experiment, randomization inference, imbalance, **problem set 2 is posted on 2/11**
...
*   - **3/2 3/4**
    - Matching
    - distance metrics, exact matching, nearest neighbor matching, propensity score matching, balancing property, sensitivity analysis, weighting, doubly robust estimators, **problem set 3 is posted on 2/25**
...
*   - **3/16**
    - Matching (cont.)
    - Weighting, doubly robust estimators, sensitivity analysis
    -   - @sekhon2009opiates
        - @caliendo2008some
    
*   - **3/18 3/23**
    - Instrumental Variables
    - Local Average Treatment Effects, IV estimator, one-/two-sided non-compliance, 2SLS estimator and its bias, **experimental replication is due on 3/16**, **problem set 4 is posted on 3/18**
...
*   - **3/25 3/30**
    ...
    - two-period DID, pre-trends, event study, conditional DID, continuous treatment DID, event-by-event estimation, **problem set 5 is posted on 3/30**
...
*   - **4/8 4/13**
    ...
    - parametric, non-parametric local regression, optimized RD and honest inference, threats, fuzzy RDD, **problem set 6 is posted on 4/13**
...
*   - **4/15 4/20**
    - Mediation Effects
    - natural direct and indirect effects, controlled direct and indirect effects, **observational replication is due on 4/20**
