...
- [Important]{.alert}: In the just-identified case, $t_{LATE} = t_{ITT}$ since both estimate and SE scale by $1/\widehat{\gamma}$:
  ...
  - 2SLS will [not]{.alert} rescue an insignificant reduced form (ITT)!
...
- [Bootstrap]{.highlight} [@young2022consistency]: two methods for CIs robust to weak instruments:
  ...
  - [Bootstrap-$c$]{.highlight}: resample, re-estimate $\widehat{\tau}_{2SLS}$, take percentile CI.
  - [Bootstrap-$t$]{.highlight}: compute bootstrap $t$-statistics, form CIs via percentile-$t$ adjustments.

. . .

- [$tF$ procedure]{.highlight} [@lee2022valid]: adjusts TSLS SEs based on the first-stage $F$-statistic for just-identified IV.

  - [Idea]{.note}: standard TSLS CIs are too narrow when instruments are weak because they ignore first-stage estimation uncertainty. The $tF$ procedure widens CIs by a factor that depends on $F$.
  - [Note]{.note}: Without this adjustment, the standard $F > 10$ rule is insufficient -- maintaining a correct $5$% rejection rate would actually require $F \approx 104.7$!
...
- [Sensitivity analysis]{.highlight} [@cinelli2025omitted]:
  ...
  - How much violation of (1) randomization and (2) exclusion restriction would be needed to change results?

  - Use partial $R^2$ sensitivity framework [@cinelli2020making] varying instrument--outcome and confounder--mediator associations (recall [matching/weighting lecture]{.note}).
...
### Testing _LATE_ Assumptions in Practice
...
tibble(
  Stratum = c("Always-taker", "Never-taker", "Complier"),
  Proportion = round(c(pi_a, pi_n, pi_c), 3)
...
### Testing _LATE_: Huber Bounds Check
