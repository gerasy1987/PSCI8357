...
- The standard first-stage [$F$-statistic]{.highlight} from a regression of the endogenous regressors on instruments and exogenous controls is:
  ...
  $$
  F = \frac{R^2_{Y \sim Z}}{(1 - R^2_{Y \sim Z})} \times \frac{N - k - 1}{k} 
  $$

  where $R^2_{Y \sim Z}$ is $R^2$ from first-stage regression of $Y$ on $Z$ and $k$ is the number of instruments.
...
- Given the monotonicity assumption, we can estimate proportions of all principal strata:
  ...
  $$
  \begin{align*}
  \pi_{\text{a}} &= \Pr(\text{always-takers}) = \Pr(T_i = 1, Z_i = 0) \\
  \pi_{\text{n}} &= \Pr(\text{never-takers}) = \Pr(T_i = 0, Z_i = 1) \\
  \pi_{\text{c}} &= \Pr(\text{compliers}) \\
                 &= \Pr(T_i = 1, Z_i = 1) - \pi_{\text{a}} = \Pr(T_i = 0, Z_i = 0) - \pi_{\text{n}}
  \end{align*}
  $$
