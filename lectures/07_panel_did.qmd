...
where $N_1$ and $N_0$ are treated and control unit counts. **Inference**: SE from standard difference in means.
...
where $N_{gt}$ is the number of units in Group $g$ at time $t$. **Inference**: SE from standard difference in means.
...
cat(
  "Effect of switching to on-cycle elections on log avg. teacher salary (Anzia 2012):", round(rcs_estimate, 4))
```
...
- Stationarity is satisfied and $F(y_i \given G_i = 0)$ stochastically dominates $F(y_i \given G_i = 1)$ (reversed dominance $\rightsquigarrow$ reversed bracketing).
...
<iframe width="450" height="250" src="https://www.youtube.com/embed/6nG63ISt_Ek?start=26" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen data-external="1"></iframe>
...
- Suppose that treatment is continuous and (possibly) non-linear:
  ...
  $$
  Y_{it} = \alpha_i + \lambda_t + \tau G_i \times \text{Post}_t \times f(T_it)  + \varepsilon_{it},
  $$
