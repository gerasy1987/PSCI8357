...
- [Intuition]{.note}: No bias when **no dynamic tretament effects** (no HTE over time, i.e., $\tau(1, 1) = \tau(1, 0)$). [**Moreover returns ATT when no HTE at all!**]{.fragment}
...
- [Synthetic Control (SCM)]{.highlight}: Assume untreated trend is an [interactive FE (factor) model]{.highlight}:
  ...
  $$
  Y_{it}(0) = \gamma_t + \lambda_t^\prime \pmb{\mu_i} + \beta^\prime \mathbf{X}_i + \varepsilon_{it},
  $$
