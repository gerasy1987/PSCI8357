...
### {data-visibility="uncounted"bility="hidden"}
...
panelView::panelview(
  ...
  xlab = "Year", ylab = "State", type = "treat", theme.bw = FALSE,
  display.all = T, gridOff = TRUE, by.timing = TRUE, collapse.history = "TRUE", axis.lab.angle = 90
  ) + custom_theme
...
- [Contemporaneous _ATT_]{.highlight}: The _ATT_ of the treatment at time $t$ on outcome at time $t$:  
  $$
  \tau(t) = \E [Y_{it}(1) - Y_{it}(0) \mid G_i = t].
  $$
...
- [Long-term _ATT_]{.highlight}: The _ATT_ of the treatment at time $t$ on outcome at time $t+s$:  
  $$
  \tau(t, s) = \E [Y_{i,t+s}(1) - Y_{i,t+s}(0) \mid G_i = t].
  $$
...
- **Assumption**: [Parallel Trends at time $t$]{.highlight} 
  $$
  \E [Y_{it}(0) - Y_{i,t-1}(0) \mid G_i = t] = \E [Y_{it}(0) - Y_{i,t-1}(0) \mid G_i > t].
  $$
...
- **Assumption**: [(long-term) Parallel Trends at time $t$]{.highlight}  
  $$
  \E [Y_{i,t+s}(0) - Y_{i,t-1}(0) \mid G_{i} = t] = \E [Y_{i,t+s}(0) - Y_{i,t-1}(0) \mid G_{i} > t+s].
  $$
...
summary(fit_sa, estimator = "SA-DID") |> as_tibble() |> dplyr::select(1:4) |> 
...
summary(fit_sa_dyn, estimator = "SA-DID") |> as_tibble() |> dplyr::select(1:4) |>
  knitr::kable(digits = 4, align = "lccccccc") |>
  kableExtra::kable_minimal(font_size = 20)
...
- Nonparametric DiD estimators are direct extensions of the basic DiD case, and they are transparent and easy to compute. 

  - However, they could be less efficient.

- The most popular approach is the [Two-Way Fixed Effects Estimator (TWFE)]{.highlight}:
  $$
  Y_{it} = \alpha_i + \gamma_t + \tau T_{it} + (\mathbf{X}_{it}^\top \gamma) + \epsilon_{it}
  $$
...
- Angrist and Pischke (2008) refer to the two-way fixed effects estimator as the "generalized" difference-in-differences estimator.
  - Equivalent under the basic DiD design (two groups and two time periods).

- However, a series of recent papers reveal that the two-way fixed effects estimator is biased for the ATT when the treatment effects vary over time.

### Unpacking Two-way Fixed Effects Estimator
...
![](../_images/bacon_1.png){width="70%" fig-align="center"}

- **Three groups**: 
  - $k$ treated at $t^\star_k$ (or **Group $1$**),
  - $\ell$ treated at $t^\star_\ell$ (or **Group $2$**),
  - $U$ untreated (or **Group $\infty$**).

### @goodman2021difference: Three Groups under SA Design

![](../_images/bacon_2.png){width="70%" fig-align="center"}

- Consider all of the $2 \times 2$ DiDs you could construct.  
- 3 different ATT estimates (panels A–C), and then a fourth _inverse_ ATT (panel D).  
- **Question**: How does TWFE relate to these effects?

### Unpacking Two-way Fixed Effects Estimator (Continued)
...
- [Parallel Trends Assumption]{.highlight}:
  ...
  - The parallel trends assumption holds for *all* groups for all time periods:
    
    $$
    \forall g,t: \: \E[Y_{it}(0) \mid G_i = g] - \E[Y_{i,t-1}(0) \mid G_i = g].
    $$
...
- [Intuition]{.note}: No bias when no treatment effect heterogeneity over time (i.e., $\tau(1, 1) = \tau(1, 0)$)

### Two-way Fixed Effects with "Dynamic Specification"

- One "partial" solution: Dynamic TWFE (Angrist and Pischke, 2008):
  $$
  \begin{align*}
  ...
  - $\tau_0$ captures the contemporaneous effect.  
  - $(\tau_1, \ldots, \tau_S)$ capture the long-term effect.  
  - $\widetilde{\tau}_S$ captures the (very) long-term effect altogether.

- $S$ determines how far post-treatment periods we want to examine.
    
    $\rightsquigarrow$ Relax the constant treatment effect assumption.

- Note that Sun and Abraham (2021) show that this does not fully solve the treatment effect heterogeneity.
