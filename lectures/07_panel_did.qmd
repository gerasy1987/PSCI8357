...
- The [target estimand]{.highlight} is $\overline{\tau}^{\text{SA}}$, the treated-observation-weighted average of contemporaneous ATTs: $\overline{\tau}_{} = \sum_{t=1}^T \tau(t, 0) \ \frac{\sum_{i=1}^n \mathbb{1}\{G_i = t\}}{\sum_{i=1}^n \mathbb{1}\{G_i \leq T\}}.$

. . .

- [Bias]{.alert}: $\E(\widehat{\tau}_{\text{SA}}) \neq \overline{\tau}^{\text{SA}}$ whenever $\tau(1,1) \neq \tau(1,0)$. The weight on $\tau(1,1)$ is $(\omega_{1\infty}/2 - \omega_{12}/2)$, which can even be [**negative**]{.alert} when $\omega_{12} > \omega_{1\infty}$ (i.e., more early-vs-late units than early-vs-never).

. . .

- [Intuition]{.note}: No downward bias when **no dynamic treatment effects** (no HTE over time, i.e., $\tau(1, 1) = \tau(1, 0)$). **Moreover returns $\overline{\tau}^{\text{SA}}$ when no HTE at all!**
...
- [Dynamic TWFE with leads and lags]{.highlight} [@angrist2009mostly]:  
  ...
  - $(\delta_1, \ldots, \delta_Q)$ should be $0$ if the parallel trends assumption holds.
...
- **Estimation**: Linear model with OLS, explicitly conditioning on history and covariates:
  $$
  Y_{it} = \alpha + \tau T_{it} + \mathbf{X}_{it}^\prime \beta + \sum_{\ell=1}^{L} \rho_\ell Y_{i,t-\ell} + \sum_{\ell=1}^{L} \delta_\ell T_{i,t-\ell} + \gamma_t + \varepsilon_{it}
  $$

  - $\mathbf{X}_{it}$: observed (time-varying) covariates.

  - Cluster standard errors at the unit level.

. . .

- [Why this solves the feedback problem]{.note}: SI [explicitly conditions on the treatment-outcome history]{.highlight} $\rightsquigarrow$ blocks feedback paths $(Y_{i,t-1} \to T_{it} \to Y_{it})$ by including lagged $Y$ and $T$ as covariates.

- [Trade-off]{.alert}: No unit FE $\rightsquigarrow$ any time-invariant _unobserved_ confounder is a threat.
...
- This implies untreated trend is a [TWFE process]{.highlight} $Y_{it}(0) = \alpha_i + \gamma_t + \mathbf{X}_{it}^\prime \beta + \varepsilon_{it}.$
...
- [Note]{.note}: If $\lambda_t = \lambda$ (constant over time), then $\lambda^\prime \pmb{\mu}_i$ is time-invariant $\rightsquigarrow$ absorbed into a unit FE $\alpha_i \equiv \lambda^\prime \pmb{\mu}_i$, and the model collapses to TWFE. **SCM generalizes DiD by allowing [time-varying factor loadings]{.highlight}.**
...
- [Balancing weights]{.highlight} cause the $X_j$ term to cancel out when we take $Y_{1t}(0) - \sum_{j=2}^{J+1} w_j^\ast Y_{jt}$ for $t \leq T_{0}$ $\rightsquigarrow$ use them to approximate $Y_{1t}(0)$ for $t > T_{0}$.
...
- [Optimisation problem]{.highlight}: Choose synthetic weights $\mathbf{W}^\ast$ such that 
  ...
  $$
  | X_{1t} - \sum_{j=2}^{J+1} w^\ast_j X_{jt}|, \quad | Y_{1t} - \sum_{j=2}^{J+1} w^\ast_j Y_{jt} |.
  $$
...
- [Analogy with matching/weighting]{.note}: SCM $\approx$ [covariate balancing]{.highlight} — find convex weights on donors to match treated unit's pre-treatment profile.

. . .

- [Key difference from matching]{.highlight}: non-negative convex weight constraint $\rightsquigarrow$ [interpolation]{.highlight}, making donor quality transparent.

. . .

- [Nested optimization]{.highlight} [@abadie2010synthetic]:

  - [Inner loop]{.note} (given $V$): choose $\mathbf{W}^\ast(V)$ to minimize weighted predictor distance
  $$
  \mathbf{W}^\ast(V) = \arg\min_{\mathbf{W} \in \mathcal{W}} \; (\mathbf{X}_1 - \mathbf{X}_0 \mathbf{W})' V (\mathbf{X}_1 - \mathbf{X}_0 \mathbf{W})
  $$
  where $\mathcal{W} = \{w_j \geq 0, \sum_j w_j = 1\}$ — a convex QP.

. . .

  - [Outer loop]{.note}: choose $V^\ast$ to minimize pre-treatment MSPE
  $$
  V^\ast = \arg\min_{V \succeq 0} \; \frac{1}{T_0}\sum_{t=1}^{T_0}\left(Y_{1t} - \sum_{j=2}^{J+1}w_j^\ast(V) \, Y_{jt}\right)^2
  $$
  solved numerically (e.g., Nelder-Mead) — $V$ diagonal, entries = predictor importance weights.

. . .

- [Intuition]{.note}: data-driven predictor weighting — $V$ upweights covariates that help track pre-treatment outcomes, downweights those that don't.
