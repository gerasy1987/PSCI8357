...
- Estimation: flexible first-difference model for $m(d) = \E[\Delta Y_i \given G_i = d]$
  ...
  $$
  \Delta Y_i = \alpha + m(T_{i1}) + \varepsilon_i
  $$
...
- Common TWFE for dose-response summary:
  ...
  $$
  Y_{it} = \alpha_i + \lambda_t + \tau T_{it} + u_{it}
  $$
...
- [Option: binarize treatment]{.highlight}
  ...
  $$
  \Delta Y_i = \alpha + \tau_{bin} G_i + \varepsilon_i
  $$
...
- The most popular approach is the [Two-Way Fixed Effects Estimator (TWFE)]{.highlight}:
  $$
  Y_{it} = \alpha_i + \gamma_t + \tau T_{it} + (\mathbf{X}_{it}^\prime \beta)  + \varepsilon_{it}
  $$

- The OLS coefficient can be estimated as (implemented by `fixest`, `plm`, `estimatr`...):
  $$
  (\hat{\tau}_{\text{SA}}, \widehat{\pmb{\alpha}}, \widehat{\pmb{\gamma}}, \widehat{\pmb{\beta}}) = \argmin_{\tau, \pmb{\alpha}, \pmb{\gamma}, \pmb{\beta}} \frac{1}{T \ N} \sum_{t=1}^T \sum_{i=1}^N (Y_{it} - \alpha_i - \gamma_t - \tau T_{it} - \mathbf{X}_{it}^\prime \beta)^2
  $$
