...
- Suppose $w^\star$ for which:

  $$
  \sum_{j=2}^{J+1} w_j^\star \pmb{\mu}_j = \pmb{\mu}_1, 
  \quad \text{and} \quad 
  \sum_{j=2}^{J+1} w_j^\star \mathbf{X}_j = \mathbf{X}_1.
  $$

. . .

- These [balancing weights]{.highlight} cause the $X_j$ term to cancel out when we take $Y_{1t}(0) - \sum_{j=2}^{J+1} w_j^\star Y_{jt}$ for $t > T_{0}$ $\rightsquigarrow$ approximately $\tau_{1t}$.
...
- [Optimisation problem]{.highlight}: Choose synthetic weights $\mathbf{W}^\star$ such that 

  - $w_i \geq 0$ for $i \in \{2,...,J+1\}$ and $\sum_{i=2}^{J+1} w_i = 1$,
  - $\forall t < T_0$ (pre-treatment periods) $\mathbf{W}^\star$ minimizes
  
  $$
  | X_{1t} - \sum_{j=2}^{J+1} w^\star_j X_{jt}|, \quad | Y_{11} - \sum_{j=2}^{J+1} w^\star_j Y_{j1} |.
  $$
