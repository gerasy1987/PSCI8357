...
- [DiD]{.highlight}: Focus on ATT and assume parallel trends.
...
- This implies untreated trend is a [TWFE process]{.highlight} $Y_{jt}(0) = \alpha_j + \gamma_t + \varepsilon_{jt}.$

. . .

- [Synthetic Control (SCM)]{.highlight}: Estimand is effect trend for $i = 1$, $(\tau_{1,T_0+1}, \dots, \tau_{1,T})$

  $$
  \tau_{1t} = Y_{1t}(1) - Y_{1t}(0) = \underbrace{Y_{1t}}_{\text{Observed}} - \underbrace{Y_{1t}(0)}_{\text{Counterfactual}}
  $$

- Assumption: untreated trend is an [interactive FE (factor) model]{.highlight}:

  $$
  Y_{it}(0) = \gamma_t + \lambda_t^\prime \mu_i + \beta^\prime \mathbf{X}_i + \varepsilon_{it},
  $$
...
- Weighted sums of control observations is then:
  ...
  = \gamma_t + \beta_t^\prime \sum_{j=2}^{J+1} w_j \mathbf{X}_j + \lambda_t^\prime \sum_{j=2}^{J+1} w_j \mu_j + \sum_{j=2}^{J+1} w_j \varepsilon_{jt}
  $$
...
- Suppose $w^\star$ for which:
  ...
  $$
  \sum_{j=2}^{J+1} w_j^\star Y_{jt} = Y_{1t} \quad \text{for } t \leq T_0, 
  \quad \text{and} \quad 
  \sum_{j=2}^{J+1} w_j^\star X_j = X_1.
  $$

. . .

- These [balancing weights]{.highlight} cause the $X_j$ term to cancel out when we take $Y_{1t}(0) - \sum_{j=2}^{J+1} w_j^\star Y_{jt}$.

## Estimation and Inference

### Estimation

- $\mathbf{W} = (w_2,\dots, w_{J+1})^\prime$: Synthetic weights for the control units.
