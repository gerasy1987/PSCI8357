...
- **Estimand:** _ATT_ in the post-treatment period  
  ...
  pbapply::pbreplicate(1000,
    {
      ...
      unit_shocks <- runif(n, 0, 0.5)

      sim_df <- 
        tibble(
          id = rep(1:n, each = 2),
          time = rep(c(0, 1), times = n),
          treat = as.numeric(id %in% sample(1:n, size = n/2, prob = 1 / 4 + unit_shocks)),
          ## time-invariant confounding
          U_ti = rep(unit_shocks, each = 2),
...
- Two additional ways produce equivalent estimates in the two-period case without controls:
  ...
  - [Two-Way Fixed Effects model]{.highlight}: $Y_{it} = \alpha_i + \lambda_t + \tau G_{it} + \varepsilon_{it}$
  - [First differences model]{.highlight} (only in panel case): $\Delta Y_i = \Delta \lambda + \tau G_i + \Delta \varepsilon_i$
...
- Researchers often add covariates to TWFE or interactive specifications, e.g.:
  ...
  - This makes the following assumptions:
    ...
    - [conditional parallel trends]{.highlight}.
...
- **Approach 2**: [Formal tests]{.highlight}
  ...
  2. Estimate full $t \times G$ interactive model with $t = 0$ as reference category.
