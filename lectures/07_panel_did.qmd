...
- The [target estimand]{.highlight} is $\overline{\tau}^{\text{SA}}$, the treated-observation-weighted average of contemporaneous ATTs:
  $$
  \overline{\tau}_{} = \sum_{t=1}^T \tau(t, 0) \ \frac{\sum_{i=1}^n \mathbb{1}\{G_i = t\}}{\sum_{i=1}^n \mathbb{1}\{G_i \leq T\}}.
  $$
...
- [Intuition]{.note}: No bias when **no dynamic treatment effects** (no HTE over time, i.e., $\tau(1, 1) = \tau(1, 0)$). **Moreover returns $\overline{\tau}^{\text{SA}}$ when no HTE at all!**
...
- [Why this solves the feedback problem]{.note}: TWFE uses unit FE $\alpha_i$ to absorb all time-invariant confounding, but [cannot condition on past outcomes]{.highlight} (they are post-treatment for earlier periods). SI [explicitly conditions on the treatment-outcome history]{.highlight} $\rightsquigarrow$ blocks feedback paths $(Y_{i,t-1} \to T_{it} \to Y_{it})$ by including lagged $Y$ and $T$ as covariates.

- [Trade-off]{.alert}: No unit FE $\rightsquigarrow$ any time-invariant unobserved confounder is a threat.
...
- [Analogy with matching/weighting]{.note}: SCM is essentially a [covariate balancing]{.highlight} problem -- find weights on donor units to match the treated unit.

. . .

- $\mathbf{W}$ is chosen to minimize the distance between treated unit's and weighted donors' pre-treatment profiles, subject to $w_j \geq 0$ and $\sum_{j} w_j = 1$ (convex combination).

. . .

- [Key difference from matching]{.highlight}: the non-negative convex weight constraint $\rightsquigarrow$ synthetic control is an [interpolation]{.highlight}, also making quality of donor units transparent.

. . .

- @abadie2010synthetic choose $\Sigma$ (importance weights across predictors) to minimize the Mean Squared Prediction Error (MSPE) for pre-treatment outcomes:
  $$
  \text{MSPE} \ = \ \frac{1}{T_0}\sum_{t=1}^{T_0}\left(Y_{1t} - \sum_{j=2}^{J+1}w_j^\ast Y_{jt}\right)^2.
  $$
