...
- **Potential Outcome**: $Y_{it}(t)$ where $t \in \{0,1\}$.
...
- Suppose $w^\ast$ for which:

  $$
  \sum_{j=2}^{J+1} w_j^\ast \pmb{\mu}_j = \pmb{\mu}_1, 
  \quad \text{and} \quad 
  \sum_{j=2}^{J+1} w_j^\ast \mathbf{X}_j = \mathbf{X}_1.
  $$

. . .

- These [balancing weights]{.highlight} cause the $X_j$ term to cancel out when we take $Y_{1t}(0) - \sum_{j=2}^{J+1} w_j^\ast Y_{jt}$ for $t \leq T_{0}$ $\rightsquigarrow$ use them to approximate $Y_{1t}(0)$ for $t > T_{0}$.
...
- [HTE-robust methods for General Panel]{.highlight}
