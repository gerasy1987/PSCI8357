...
&\class{fragment}{{}= \sum_{y \in \mathcal{Y}} \left[\sum_{x \in \mathcal{X}} x \frac{p_{X,Y} (x, y)}{p_{Y} (y)} \right] p_Y (y) \quad \text{$\because$ cond. expectation definition}} \\
...
<!-- ## Interactivity: Try it Yourself!

- **Explore Sampling Distributions:** 
  - Change the sample size, mean, and standard deviation using sliders to see how the shape of the sampling distribution changes.
  - Visualize how the confidence interval narrows with increasing sample size.

```{r interactivePlot}
# R code to create interactive plots using packages like 'shiny' or 'plotly' to visualize sampling distributions
# Simulating how sample size affects the sampling distribution for a normal distribution
library(plotly)
sampleSize <- seq(1, 10000, by = 10)
meanValue <- 0
stdDev <- 1
set.seed(123)

plot_ly(
  x = sampleSize, y = rnorm(1000, mean = meanValue, sd = stdDev / sqrt(sampleSize)),
  type = "scatter", mode = "lines"
) %>%
  layout(
    title = "Change in Sampling Distribution with Sample Size",
    xaxis = list(title = "Sample Size"),
    yaxis = list(title = "Sampling Distribution of Sample Mean")
  )
``` -->
