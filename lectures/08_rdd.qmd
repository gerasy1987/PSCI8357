...
- **Identification Assumption**: [Continuity of average potential outcomes]{.highlight}
  ...
  - $\E [Y_i(t) \given X_i = x]$ is continuous in $x$ around $X_i = c \text{ for } t \in \{0,1\}$ (i.e. smooth or is $p$-times differentiable at $c$).
...
- **Observed vote share** is $V_{ijt} = V_{ijt}(T_{ijt})$.
...
1.  **Trim the sample to a _reasonable_ window** around the threshold $c$ (discontinuity sample):
...
est_diff_slope |>
  ...
  knitr::kable(
    digits = 4,
    align = "lccc",
    caption = "Estimates with Different Slopes"
  ) |>
  kableExtra::kable_minimal(font_size = 20)
```
...
- **Equivalent approach**: Regression weighted by $K(\cdot)$.
...
- **Question**: How should we choose the bandwidth $h$?
  ...
  - Larger $h$: more data $\rightsquigarrow$ lower variance, but linear approximation worsens $\rightsquigarrow$ higher bias
  - Smaller $h$: better local fit $\rightsquigarrow$ lower bias, but fewer observations $\rightsquigarrow$ higher variance

. . .

- [Idea]{.note}: Choose $h$ to minimize the [MSE of $\widehat{\tau}_{SRD}$]{.highlight}:

  $$
  \text{MSE}(\widehat{\tau}_{SRD}) \approx \underbrace{h^4 \cdot B^2}_{\text{Bias}^2} + \underbrace{\frac{V}{nh}}_{\text{Variance}}
  $$

  - Bias$^2 \propto h^4$: linear approx. error grows with $h$ ($\sim h^2$, then squared)
  - Variance $\propto 1/(nh)$: wider window $\rightsquigarrow$ more obs. $\rightsquigarrow$ lower variance

. . .

- Minimizing over $h$ $\rightsquigarrow$ [Imbens-Kalyanaraman (IK)]{.highlight} [@imbens2012optimal]: $h_{MSE} \propto n^{-1/5}$
...
- Implemented in [R]{.proglang} package `rdrobust` and reviewed in @cattaneo2022regression.
...
- A variety of threats and subtleties to be careful with (non-local or compound treatments, sorting/bunching).
...
- Threshold may not perfectly determine treatment exposure, but it may still create a **discontinuity in the probability of treatment**.

. . .

- [Hypothetical example]{.note}: SAT and scholarship
  
  - Scholarship may not be wholly determined by SAT scores $X_i$, but the rule specifies a threshold $X_i=c$ to become eligible  
  
  - Students above $c$ get a chance of receiving the scholarship, but may not actually get it  
  
  - Students below $c$ are excluded from consideration, perhaps except ones who are otherwise remarkable

. . .

- That is, an **encouragement** to receive treatment is discontinuously determined by $X_i$ at the threshold $c$.

- Fuzzy RD can therefore be thought of as an instrumental variable version of an RDD.

## Example

### Example: Class Size Effect 

- @angrist1999using study effect of class-size on student test scores.
- Maimonides' Rule: Maximum class size is $40$
  
  $$
  f(g) = \frac{g}{\left\lfloor \frac{g-1}{40} \right\rfloor + 1}
  $$

```{r}
#| label: plot_maimonaides
#| echo: true
#| code-fold: true
#| fig-align: center
#| fig-width: 9
...
df_al <- 
  read_csv("../_data/angrist_lavy_1994.csv") |> 
  dplyr::mutate(enrollment = c_size)

# compute average class size by enrollment
avg_size <- 
  df_al |>
  group_by(enrollment) |>
  summarise(avg_size = mean(classize, na.rm = TRUE), .groups = "drop")

# create predicted class size using Maimonides' Rule
enrollment_range <- 1:225
maim_rule_class_size <- enrollment_range / (floor((enrollment_range - 1) / 40) + 1)

plot_df <- avg_size |>
  dplyr::filter(enrollment <= 225) |>
  dplyr::right_join(
    tibble(enrollment = enrollment_range, maim_rule = maim_rule_class_size),
    by = "enrollment"
  )

plot_df |>
  # filter(!is.na(avg_size) & !is.na(maim_rule)) |>
  ggplot(aes(x = enrollment)) +
  geom_line(aes(y = avg_size), data = filter(plot_df, !is.na(avg_size)), color = "#458588", size = 1.5, alpha = .8) +
  geom_line(aes(y = maim_rule), data = plot_df, linetype = "dashed", color = "#cc241d", size = 1, alpha = .8) +
  labs(
    x = "Enrollment Count",
    y = "Class Size"
  ) +
  geom_vline(xintercept = seq(40, 200, by = 40), linetype ="dashed", size = .5) +
  annotate("text", x = 200, y = 42, label = "Maimonides Rule", color = "#cc241d", hjust = 1, size = 6) +
  annotate("text", x = 200, y = 27, label = "Actual class size", color = "#458588", hjust = 1, size = 6) +
  scale_y_continuous(limits = c(0, NA)) +
  scale_x_continuous(limits = c(0, 225), breaks = seq(0, 200, by = 40)) +
...
- **Forcing variable** can be constructed as
  
  $$
  X_i = 
  \begin{cases}
    G_i - 40 & \text{if } 40 - w/2 \leq G_i \leq 40 + w/2 \\
    G_i - 80 & \text{if } 80 - w/2 \leq G_i \leq 80 + w/2 \\
    \vdots & \vdots
  \end{cases}
  $$
...
- Class size $T_i$ discontinuously decreases at the cutoffs.

. . .

- [Note]{.note}: This example is more complex than standard fuzzy RD [with multi-cutoffs and discrete forcing variable]{.highlight}.

## Setup and Identification

### Fuzzy RD: Setup and Identification

- $Z_i \in \{0,1\}$: Encouragement
- $X_i$: Forcing variable that perfectly determines the value of $Z_i$ with cutpoint $c$
...
- **Identification assumptions:**
  1. Both $\E [T_i(z) \given X_i=x]$ (potential treatment) and $\E [Y_i(z) \given X_i=x]$ (potential outcome) are [continuous]{.highlight} in $x$ around $X_i=c$ for $z \in \{0,1\}$.
  2. [IV assumptions]{.highlight}: Monotonicity, exclusion restriction, relevance of $Z_i$

- **Causal estimand:** Local ATE for compliers at the threshold
  $$
  \tau_{FRD} = \E [Y_i(t=1) - Y_i(t=0) \given \text{unit $i$ is a complier and } X_i=c]
  $$

. . .

- **Identification Result:** Under the above assumptions,
  $$
  \tau_{FRD} = \frac{\lim_{x \downarrow c} \E [Y_i \given X_i = x] - \lim_{x \uparrow c} \E [Y_i \given X_i = x]}{\lim_{x \downarrow c} \E [T_i \given X_i = x] - \lim_{x \uparrow c} \E [T_i \given X_i = x]}
  $$

### Fuzzy RD: Discontinuity in $\E [T \given X]$

![](../_images/rdd_fuzzy1.png){width="70%" fig-align="center"}

### Fuzzy RD: Discontinuity in $\E [Y \given X]$

![](../_images/rdd_fuzzy2.png){width="70%" fig-align="center"}

### Fuzzy RD: Discontinuity in $\E [Y \given X]$

![](../_images/rdd_fuzzy3.png){width="70%" fig-align="center"}

### Fuzzy RD: Discontinuity in $\E [Y \given X]$

![](../_images/rdd_fuzzy4.png){width="70%" fig-align="center"}

### Fuzzy RD: Continuity in $\E [Y(t) \given X]$

![](../_images/rdd_fuzzy5.png){width="70%" fig-align="center"}

### Fuzzy RD: Discontinuity in $\E [Y \given X]$

![](../_images/rdd_fuzzy6.png){width="70%" fig-align="center"}

### Proof: Identification under Fuzzy RDD

- First focus on the numerator:
  
  $$
  \begin{align*}
  &\lim_{x \downarrow c} \E [Y_i \given X_i = x] - \lim_{x \uparrow c} \E [Y_i \given X_i = x] \\
  &= \lim_{x \downarrow c} \E [Y_i(z=1) \given X_i = x] - \lim_{x \uparrow c} \E [Y_i(z=0) \given X_i = x] \\
  &= \E [Y_i(z=1) \given X_i = c] - \E [Y_i(z=0) \given X_i = c] \\
  &= \left\{ \E [Y_i(t=1) \given S_i = \text{co}, X_i = c] - \E [Y_i(t=0) \given S_i = \text{co}, X_i = c] \right\}  \textcolor{#cc241d}{\Pr(S_i = \text{co} \given X_i = c)}
  \end{align*}
  $$
  
  where $S_i = \text{co}$ indicates that unit $i$ is a **complier**.

- Next focus on the denominator:
  
  $$
  \begin{alignat*}{2}
  &\lim_{x \downarrow c} \E [T_i \given X_i = x] - \lim_{x \uparrow c} \E [T_i \given X_i = x] \\
  &\quad= \lim_{x \downarrow c} \E [T_i(z=1) \given X_i = x] - \lim_{x \uparrow c} \E [T_i(z=0) \given X_i = x] \\
  &\quad= \E [T_i(z=1) \given X_i = c] - \E [T_i(z=0) \given X_i = c] \\
  &\quad= \textcolor{#cc241d}{\Pr(S_i = \text{co} \given X_i = c)} \qquad \qquad \qed
  \end{alignat*}
  $$

## Estimation

### Estimation and Inference under Fuzzy RDD

1. Trim the sample to a reasonable window above and below the threshold $c$ (discontinuity sample)

2. Code the encouragement indicator: $Z_i = \mathbb{1}\{X_i > c\}$

3. Recode the forcing variable to deviation from $c$: $\tilde{X}_i = X_i - c$

4. Estimate the outcome model using two-stage least squares (2SLS):

  $$
  \begin{align*}
  T_i &= \alpha_0 + \alpha_1 \tilde{X}_i + \alpha_2 Z_i + \alpha_3 Z_i \tilde{X}_i + \nu_i \quad \text{(First Stage)} \\
  Y_i &= \beta_0 + \beta_1 \tilde{X}_i + \beta_2 Z_i \tilde{X}_i + \tau T_i + \epsilon_i \quad \text{(Second Stage)}
  \end{align*}
  $$

. . .

- More flexible specifications can be used (e.g. polynomials of $\tilde{X}_i$).

- Then $\hat{\tau}_{2SLS}$ consistently (but not unbiasedly) estimates $\tau_{FRD}$.

- In addition, it is also helpful to separately plot (and estimate) the outcome discontinuity and treatment discontinuity for interpretation.

- Usual diagnostics can be applied to check plausibility of assumptions.

### Example: IV vs Treatment
...
df_al_bwcut <-
  df_al |>
  dplyr::filter(
    (enrollment >= 36 & enrollment <= 46) | (enrollment >= 76 & enrollment <= 86) |
      (enrollment >= 116 & enrollment <= 126) | (enrollment >= 156 & enrollment <= 166) |
      (enrollment >= 196 & enrollment <= 206)
  ) |>
  dplyr::mutate(
    forcing = case_when(
      enrollment <= 46 ~ enrollment - 41,
      enrollment <= 86 ~ enrollment - 81,
      enrollment <= 126 ~ enrollment - 121,
      enrollment <= 166 ~ enrollment - 161,
      TRUE ~ enrollment - 201
    )
  )

rdplot(y = df_al_bwcut$classize, x = df_al_bwcut$forcing, p = 1, y.label = "Class Size", x.label = "Enrollment around Cutoffs")

```

### Example: IV vs Outcome

```{r}
#| label: plot_iv_outcome
#| echo: true
#| code-fold: true
#| fig-align: center
#| fig-width: 8
...
rdplot(y = df_al_bwcut$avgverb, x = df_al_bwcut$forcing, p = 1, y.label = "Class Average Exam Score", x.label = "Enrollment around Cutoffs")

```


### Example: Class Size Eﬀect

<br>

```{r}
#| label: est_fuzzy_estimatr
#| echo: true
#| eval: true
#| output-location: column-fragment
#| output: asis
...
df_al_bwcut <-
  df_al_bwcut |>
  dplyr::mutate(iv = ifelse(forcing >= 0, 1, 0))

fuzzy_iv_fit <-
  estimatr::iv_robust(
    avgverb ~ classize + forcing + forcing:iv |
      forcing + iv + forcing:iv,
    data = df_al_bwcut, cluster = schlcode
  )

broom::tidy(fuzzy_iv_fit) |>
  dplyr::select(1:3, 5) |>
  knitr::kable(
    digits = 4,
    align = "lccc",
    caption = "Fuzzy RD Estimates"
  ) |>
  kableExtra::kable_minimal(font_size = 20)
...
fuzzy_fit <-
  rdrobust::rdrobust(
    y = df_al_bwcut$avgverb, x = df_al_bwcut$forcing, fuzzy = df_al_bwcut$classize,
    kernel = "uniform", p = 1, h = 5,
    # use masspoints = "ignore" because forcing variables are discrete
    masspoints = "off", cluster = df_al_bwcut$schlcode
  )

tibble(
  estimator = rownames(fuzzy_fit$coef),
  estimate = as.vector(fuzzy_fit$coef),
  std.error = as.vector(fuzzy_fit$se)
) |>
  knitr::kable(
    digits = 4,
    align = "lcc",
...
### Diagnostics: Falsification Checks

<br><br>

- Diagnose the robustness of your results via **falsification checks**
- The same idea as the Sharp RD case

1. **Model Specification:** Are results sensitive to alternative specifications?

2. **Placebo Test:** Does any pre-treatment covariate $W_i$ jump at the threshold?

3. **Placebo Threshold:** Check if jumps occur at placebo thresholds $c^{\ast}$?
...
- Sharp RD and Fuzzy RD exploit discontinuities in treatment **level** around a cutoff.
- Another type of discontinuity is in the treatment **slope** around a cutoff—a [_kink_]{.highlight}.

. . .

- We still assume $Y_i = \tau T_i + g(X_i) + \varepsilon_i$, but we assume that there is a kink in $T_i$:

$$
d_1 \equiv \lim_{x \downarrow c} \frac{\partial T_i}{\partial x} \neq \lim_{x \uparrow c} \frac{\partial T_i}{\partial x} \equiv d_0.
$$

- But smoothness in the trend $g(X_i)$: $\lim_{x \uparrow c} \frac{\partial g(x)}{\partial x} = \lim_{x \downarrow c} \frac{\partial g(x)}{\partial x}$; and errors, $\varepsilon_i$.

. . .

$$
\begin{align*}
\tau_{KRD} &= \frac{\lim_{x \downarrow c} \frac{\partial \E [Y_i \given X_i = x]}{\partial x} - \lim_{x \uparrow c} \frac{\partial \E [Y_i \given X_i = x]}{\partial x}}{d_1 - d_0} \\
&= \frac{\lim_{x \downarrow c} \frac{\partial (\tau T_i + g(x) + \E [\varepsilon_i \given X_i = x])}{\partial x} - \lim_{x \uparrow c} \frac{\partial (\tau T_i + g(x) + \E [\varepsilon_i \given X_i = x])}{\partial x}}{d_1 - d_0} \\
&= \frac{\tau (d_1 - d_0)}{d_1 - d_0} = \tau.
\end{align*}
$$

### Example: @landais2015assessing

![](../_images/rdd_kink1.png){width="75%" fig-align="center"}

### Example: @landais2015assessing

![](../_images/rdd_kink2.png){width="75%" fig-align="center"}


### Kink design: Estimation

<br>

- If policy rule is _deterministic_, then you can get $d_1 - d_0$ from the rule. Otherwise, a local linear specification for $i$ near $c$:

  $$
  T_i = \alpha_0 + \alpha_1 \widetilde{X}_i + \alpha_2 Z_i \widetilde{X}_i  + \nu_i,
  $$

  where $\widehat{\alpha}_2$ estimates $d_1 - d_0$.

. . .

- To estimate the kink in the outcome, again use a local linear specification:

$$
Y_i = \beta_0 + \beta_1 \widetilde{X}_i + \beta_2 Z_i \widetilde{X}_i  + \varepsilon_i,
$$

- Regression kink estimator is $\widehat{\beta}_2/(d_1 - d_0)$ if you know $d_1 - d_0$ or $\widehat{\beta}_2/\widehat{\alpha}_2$ if you have to estimate.

- See @card2017regression for generalizations and implementation in [R]{.proglang} with `rdrobust`.

### Further Extensions

<br><br>

1.  [External validity in RD]{.highlight}
  
    - @angrist2015wanna
    - @bertanha2020external

2.  [RD with multiple cutoffs]{.highlight}
    
    - @cattaneo2016interpreting

3.  [Multiway/Geographic RD Design]{.highlight}
  
    - @keele2015geographic


## Practical Consideration and Summary

### Practical Consideration

<br>

1.  **Be careful about the estimand**
    
    1. [Sharp RD]{.highlight}: Local ATE at the threshold
    2. [Fuzzy RD]{.highlight}: Local ATE at the threshold for compliers

2.  **Provide falsification tests to evaluate assumptions**
  
    - Check modeling assumptions: Model Specification
    - Check continuity assumptions (identification)
      
      1. Placebo Test
      2. Placebo Threshold
      3. Sorting

3. **Whenever possible, provide both visualization and formal tests**
...
- **Advantages:**
  ...
  1. The design is simple and it is easy to communicate your RDD
  2. Visualization can often provide clear evidence
  3. A series of well-known falsification tests are available

. . .

- **Disadvantages:**
  
  1.  Very limited external validity: at best, RDD estimates the average effect of the sub-population with $X_i$ close to the cutoff.
      - Only with additional assumptions (e.g. constant treatment effects), we can estimate the overall ATE.
  
  2. External validity is even more limited for the fuzzy RD.
