...
### A Classic Example: Effect of Scholarship

<br>

- @thistlethwaite1960regression study the effects of college scholarships on later students' achievements.
...
- @lee2008randomized studies the effect of incumbency status on vote shares?

. . .

- **Treatment**: Incumbency status of party $j$ in district $i$ at election $t$, $T_{ijt}$.
- **Potential outcomes**: Potential vote share as incumbent ($d=1$) or challenger ($d=0$), $V_{ijt}(d)$.

- **Observed vote share**: $V_{itj} = V_{ijt}(T_{ijt})$.

- Party incumbency effect for party $j$ in election $t$: $V_{ijt}(1) - V_{ijt}(0)$.

. . .

- (Observed) margin of victory for party $j$: $Z_{ijt} = V_{ijt} - V_{ikt}$ where $k \neq j$ indicates the strongest opposition party.
...
1.  **Trim the sample to a reasonable window (bandwidth)** around the threshold $c$ (discontinuity sample):
  
    - $c-h \leq X_i \leq c + h$, where $h > 0$ determines the width of the window.
...
3.  **Decide on a model** for $\E [Y_i \given \widetilde{X}_i]$:
    ...
    - Linear, common slope for $\E(Y_i \given \widetilde{X}_i < 0)$ and $\E(Y_i \given \widetilde{X}_i > 0)$.
    - Linear, different slopes.
    - Local linear with kernel weights.
...
### LM with Common Slope: Illustration
...
### LM with Different Slopes: Illustration
...
- Or weight regression with a $K(\cdot)$ [kernel function]{.highlight} of choice
  - **Uniform kernel**: $K(u) = \mathbb{1}\{|u|<1\}$.
  - **Triangular kernel**: $K(u) = (1-|u|)\mathbb{1}\{|u| < 1\}$.
  - **Epanechnikov kernel**: $K(u) = (1 - u^2)\mathbb{1}(|u| \leq 1)$.

- [Note]{.note}: You want to show the robustness across different kernel weights.
...
df_lee |>
  ...
  ggplot(data = _, aes(x = margin_tm1, y = share_t, color = factor(incumbent))) +
  geom_point(aes(size = kernel_weight), alpha = 0.2, shape = 21) +
  geom_function(
    fun = base::Vectorize(function(x) {
      if (x < 0) {
...
- [Imbens-Kalyanaraman (IK) algorithm]{.highlight} [@imbens2012optimal]:
...
- **Problem**: Local linear regression is an *approximation* $\rightsquigarrow$ need to use a confidence interval robust to bias and adjust the bandwidth selection [e.g. @calonico2014robust].
...
- We diagnose the robustness of your results via [falsification checks]{.highlight}:
...
- **Approach 2 ([Placebo test]{.highlight}):**  Use $W_i$ as a **placebo outcome**.  
  ...
  - The causal effect should be zero if the RDD is valid.
...
- **Related problem**: Other treatments assigned by the exact same $X_i$ and $c$ (e.g. geographic boundary).
...
- **Approach 2 (Formal test):** Density test
...
- Discussion of **close elections controversy** [@caughey2011elections; @de2016misunderstandings; @eggers2015validity]
...
- A variety of threats and subtleties to be careful with (treatments, sorting).

- Estimation is also tricky [@stommes2023reliability].

![](../_images/stommes_2023.png){width="90%" fig-align="center"}
