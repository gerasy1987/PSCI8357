...
- [Idea]{.note}: Choose $h$ to minimize the [MSE of $\widehat{\tau}_{SRD}$]{.highlight}:
  ...
  $$
  \text{MSE}(\widehat{\tau}_{SRD}) = \text{Bias}^2(\widehat{\tau}_{SRD}) + \text{Var}(\widehat{\tau}_{SRD})
  $$

  - [Imbens-Kalyanaraman (IK) algorithm]{.highlight} [@imbens2012optimal]: $h_{MSE} \propto n^{-1/5}$
