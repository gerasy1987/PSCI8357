...
rule_input <- 1:225
maim_rule_predict <- rule_input / (floor((rule_input - 1) / 40) + 1)

plot_df <- avg_size |>
  dplyr::filter(enrollment <= 225) |>
  dplyr::right_join(
    tibble(enrollment = rule_input, maim_rule = maim_rule_predict),
    by = "enrollment"
  )
...
plot_df |>
  ...
  geom_line(aes(y = avg_size), color = "#458588", size = 1.2, alpha = .8) +
  geom_line(aes(y = maim_rule), linetype = "dashed", color = "#cc241d", size = 1, alpha = .8) +
  labs(
    x = "Enrollment Count",
    y = "Class Size"
  ) +
...
rdplot(y = df_al_bwcut$classize, x = df_al_bwcut$forcing, p = 1)
...
rdplot(y = df_al_bwcut$avgverb, x = df_al_bwcut$forcing, p = 1)
...
3.  [Geographic RD Design]{.highlight}
