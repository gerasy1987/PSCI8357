...
- [Note]{.note}: This example is more complex than standard fuzzy RD [with multi-cutoffs and descrete forcing variable]{.highlight}.
...
fuzzy_fit <-
  rdrobust::rdrobust(
    ...
    # use masspoints = "ignore" because forcing variables are descrete
...
2. **Placebo Test:** Does any pre-treatment covariate $Z_i$ jump at the threshold?
...
- We still assume $Yi = \tau T_i + g(X_i) + \varepsilon_i$, but we assume that there is a kink in $T_i$:

$$
d_1 \equiv \lim_{x \downarrow x_0} \frac{\partial D_i}{\partial x} \neq \lim_{x \uparrow x_0} \frac{\partial D_i}{\partial x} \equiv d_0.
$$

- But smoothness in the trend $g(X_i)$: $\lim_{x \uparrow x_0} \frac{\partial g(x)}{\partial x} = \lim_{x \downarrow x_0} \frac{\partial g(x)}{\partial x}$; and errors, $\varepsilon_i$.

. . .

$$
\begin{align*}
\tau_{KRD} &= \frac{\lim_{x \downarrow x_0} \frac{\partial \E [Y_i \given X_i = x]}{\partial x} - \lim_{x \uparrow x_0} \frac{\partial \E [Y_i \given X_i = x]}{\partial x}}{d_1 - d_0} \\
&= \frac{\lim_{x \downarrow x_0} \frac{\partial (\tau T_i + g(x) + \E [\varepsilon_i \given X_i = x])}{\partial x} - \lim_{x \uparrow x_0} \frac{\partial (\tau T_i + g(x) + \E [\varepsilon_i \given X_i = x])}{\partial x}}{d_1 - d_0} \\
...
- If policy rule is _deterministic_, then you can get $d_1 - d_0$ from the rule. Otherwise, a local linear specification for $i$ near $x_0$:
