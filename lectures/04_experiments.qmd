...
- **Practical Considerations**
  ...
  - In most experiments, researchers focus on the _ATE_ and use the Neyman approach to construct confidence intervals.
    ...
    - Often motivated by restrictiveness of a sharp null hypothesis (but the difference is marginal).
...
- [Pitfall 1 --- Overfitting]{.alert}: In cases where $X_i$ is weakly predictive of $Y_i$, naive adjustment introduces additional noise and can even bias estimates in small samples [@freedman2008regression].

. . .

- [Pitfall 2 --- Common-slope constraint]{.alert}: Naive adjustment forces parallel regression lines with respect to $X$ and **limits precision gains.**
...
- Further reading: @lei2021regression propose a procedure to debias Lin's estimator even in small samples.
...
# shared axis limits for visual comparison
shared_xlim <- range(X) + c(-0.3, 0.3)
shared_ylim <- range(Y) + c(-0.5, 0.5)
...
ggplot(plot_data, aes(x = X, y = Y, color = factor(D), shape = factor(D))) +
  ...
  annotate("text", x = max(X) + 0.1, y = (y_bar_0 + y_bar_1) / 2,
    label = expression(hat(tau)[DiM]), color = "#111111", size = 3) +
...
ggplot(plot_data, aes(x = X, y = Y, color = factor(D), shape = factor(D))) +
  ...
  annotate("segment", x = 5, xend = 5,
    y = a0 + 5 * bx, yend = a0 + td + 5 * bx,
    color = "#111111", linewidth = 1,
    arrow = arrow(ends = "both", length = unit(0.05, "inches"))) +
  annotate("text", x = 5.2, y = a0 + td / 2 + 5 * bx,
    label = expression(hat(tau)[naive]), color = "#111111", size = 3) +
...
ggplot(plot_data, aes(x = X, y = Y, color = factor(D), shape = factor(D))) +
  ...
  geom_function(fun = function(x) {
    adjusted_model2[1, 2] + adjusted_model2[2, 2] + (x - mean(x)) * (adjusted_model2[3, 2] + adjusted_model2[4, 2])
  }, color = "#cc241d", linewidth = 1) +
  geom_function(fun = function(x) {
    adjusted_model2[1, 2] + (x - mean(x)) * adjusted_model2[3, 2]
  }, color = "#458588", linewidth = 1) +
  geom_vline(xintercept = mean(X), linetype = "dotted", color = "#d65d0e", linewidth = 0.7) +
  annotate("segment", x = mean(X), xend = mean(X),
    y = adjusted_model2[1, 2], yend = adjusted_model2[1, 2] + adjusted_model2[2, 2],
    color = "#111111", linewidth = 1,
    arrow = arrow(ends = "both", length = unit(0.05, "inches"))) +
...
- By partitioning the population sum into subgroups, the overall _SATE_ is a share-weighted average of within-block _SATEs_:
...
- Blocking ensures balance across blocks:
  ...
  - Without block randomization, treatment might be very _imbalanced_ (in absolute terms) across $V_i$.
...
$$
\begin{align*}
\widehat{\tau}_\text{v} &= \overline{Y}_{1,\text{v}} - \overline{Y}_{0,\text{v}} = \frac{1}{N_{1,\text{v}}} \sum_{i:V_i=1} T_i Y_i - \frac{1}{N_{0,\text{v}}} \sum_{i:V_i=1} (1 - T_i) Y_i \\
\widehat{\tau}_\text{nv} &= \overline{Y}_{1,\text{nv}} - \overline{Y}_{0,\text{nv}} = \frac{1}{N_{1,\text{nv}}} \sum_{i:V_i=0} T_i Y_i - \frac{1}{N_{0,\text{nv}}} \sum_{i:V_i=0} (1 - T_i) Y_i
\end{align*}
$$
...
$$
  ...
  - **Property**: Equivalent to the regular _DiM_ if $p_\text{v} = p_\text{nv}$ (equal treatment probabilities across blocks).
  - [Note:]{.note} With unequal probabilites of assignment across block standard $\widehat{\tau}_{DiM}$ under block design will be **biased**.
...
- One can use **linear regression** to obtain unbiased estimates in block-randomized experiments.
...
- Uncertainty estimates can be obtained via [HC2 standard errors]{.highlight} or randomization inference (replicating assignment mechanism).
...
- We mentioned that block randomization with $p_j$ varying across blocks makes the naive _DiM_ **biased**, requiring either a weighted block estimator or WLS.

- This is an instance of a **general problem**: whenever units have [unequal treatment probabilities]{.highlight}, the standard _DiM_ is biased.

. . .

- Unequal probabilities can arise from:

  - Block (stratified) randomization with different $p_j$ across strata (as we just saw).

  - Cluster randomization with unequal cluster sizes.

  - Practical constraints, ethical considerations, or adaptive designs [@offer2021adaptive].

. . .

- **General solution**: Use [Inverse Probability Weighting (IPW)]{.highlight} estimator.
...
- Suppose $p_i = \Pr(T_i = 1)$ varies across units (e.g., block randomization with varying treatment probabilities across blocks), but $N_1$ and $N_0$ are fixed.
...
## CR = Randomization at the Group Level
...
- Sometimes random assignment occurs at the [cluster level]{.highlight} for various reasons:
  ...
  - Treatment only makes sense at the group level, but outcome is measured for individuals (e.g., classroom curriculum).

  - Treatment too costly to implement individually (e.g., village-level program).

  - SUTVA only plausible if treatment is defined at the group level (e.g., information campaigns).

. . .

- Cluster randomization is a natural setting where [unequal treatment probabilities]{.highlight} arise -- can use IPW estimator.

- It creates **two distinct challenges**:

  1. [Estimation]{.highlight}: When clusters have unequal sizes, DiM can be **biased** for $\tau_{ATE}$.
  2. [Inference]{.highlight}: Standard errors ignoring clustering are usually **too small**, even when DiM is unbiased.
...
- When clusters have **unequal sizes**, the _DiM_ estimator can be biased for $\tau_{ATE}$.

- [Why?]{.highlight} Even though $G_1$ (number of treated clusters) is fixed by design, $N_1 = \sum_{j: T_j=1} N_j$ (number of treated individuals) is **random** — it depends on _which_ clusters are selected.

  - _DiM_ becomes a ratio with a random denominator: $\E\!\left[\frac{\sum T_i Y_i}{N_1}\right] \neq \frac{\E[\sum T_i Y_i]}{\E[N_1]}$.

- [Intuition]{.note}: Individuals in larger clusters are _over-represented_ whenever their cluster is treated, so DiM implicitly gives more weight to larger clusters. If cluster size correlates with potential outcomes, this produces bias.
...
## Variance of DiM in Cluster-Randomized Experiments

- Even with **equal** cluster sizes (no bias), clustering _can_ inflate standard errors.

- [Intuition:]{.note} Units within the same cluster tend to be similar, so each additional unit within a cluster provides _less new information_ than an independent draw.

. . .

- To formalize this, recall the [Law of Total Variance]{.highlight}: $\var(Y) = \E[\var(Y\mid X)] + \var(\E[Y\mid X])$

- This implies the decomposition of heterogeneity in outcomes:

$$
\underbrace{\frac{1}{N}\sum_{j=1}^G \sum_{i=1}^{N_j} (Y_{ij} - \overline{Y})^2}_{\text{total variance, } \sigma^2} = \underbrace{\frac{1}{N}\sum_{j=1}^G \sum_{i=1}^{N_j} (Y_{ij} - \overline{Y}_{j})^2}_{\text{within-cluster variance, } \sigma^2_W} + \underbrace{\frac{1}{N}\sum_{j=1}^G N_{j}(\overline{Y}_{j} - \overline{Y})^2}_{\text{between-cluster variance, } \sigma^2_B}
$$

where $\overline{Y_{j}}$ is mean of $Y_{ij}$ in cluster $j$, $\overline{Y}$ is overall mean, and $N = \sum_{j=1}^G N_j$.

. . .

- Then we can define the [intracluster correlation (ICC)]{.highlight}: $\rho = \frac{\sigma^2_B}{\sigma^2} = 1 - \frac{\sigma^2_W}{\sigma^2}$

- [Intuition]{.note}: When $\rho = 1$, all variation is between clusters (units within a cluster are identical). When $\rho = 0$, cluster membership is uninformative.
...
- **Valid inference** requires [cluster-robust standard errors]{.highlight} (_CR2_) or randomization inference to account for the inflated variance.
...
![<https://gsyunyaev-psci8357-cr-inference.share.connect.posit.cloud/>](../_images/cr2_coverage.gif){fig-align="center" width="90%"}
:::
...
- Power depends on the **design** (sample size, treatment allocation, randomization scheme) and the **test** (test statistic, significance level) -- we now examine the key choices.

## Statistical Power and Variance {#optimal-p}

<br>

- Statistical power is directly linked to the variance of the estimator: **smaller variance $\Rightarrow$ higher power to reject the null.**
...
- **Problem**: For a given $N$, choose $p = N_1/N$ to minimize $\var(\hat\tau) = \frac{\sigma^2_1}{p N}+\frac{\sigma^2_0}{(1-p) N}$.

- Minimizing over $p$ gives [[(derivation)](#proof-optimal-p)]{.note .small-font}:

$$
p^* = \frac{\sigma_1}{\sigma_1+\sigma_0}=\frac{1}{1+\sigma_0/\sigma_1}
$$

- [Intuition]{.note}: If $\sigma_1\approx \sigma_0$, then $p^{*}=0.5$. For _practical_ reasons it is sometimes better to choose unequal sample sizes (even if $\sigma_1\approx \sigma_0$).
...
## Choice of Overall Sample Size ($N$) {#power-formula}

- Under a two-sided test at level $\alpha = 0.05$, the power function is [[(derivation)](#proof-power-formula)]{.note .small-font}:
...
:::fragment
- [Flipside]{.highlight}: Given a fixed design (including $N$, $p$, $\sigma^2$), what is the smallest effect size we can reliably detect?
:::
:::
...
- Let $\sigma_{\hat{\beta}} = \max\bigl(\sigma_{\hat{\beta}}^{(0)}, \sigma_{\hat{\beta}}^{(a)}\bigr)$, then define a conservative _MDE_ as
...
## MDE and Sample Size Determination

- **Practical question**: Given a target MDE, how large must $N$ be?

- Under CRA, $\;\var[\hat\beta] = \frac{1}{N}\!\left(\frac{\sigma_1^2}{p} + \frac{\sigma_0^2}{1-p}\right)$, so $MDE = 2.8 \times \underbrace{\sqrt{\frac{1}{N}\!\left(\frac{\sigma_1^2}{p} + \frac{\sigma_0^2}{1-p}\right)}}_{SE(\hat\beta)}$.
...
$$
N = \frac{2.8^2 \left( \frac{\sigma^2_{1}}{p} + \frac{\sigma^2_{0}}{1 - p} \right)}{MDE^2}
$$

- [Key insight]{.note}: $N$ scales as $1/MDE^2$ -- halving the detectable effect requires **4 times** the sample size.

. . .

- [Be vary of "power fallacy"]{.alert}: Raw MDE depends on $\sigma$ -- a noisy outcome variable makes MDE look large even with a large $N$. Always report standardized effect sizes, e.g. [*Glass's* $\Delta$]{.highlight} $= MDE / \sigma_0$.

## Conclusion on Power

- [Statistical power]{.highlight}  is the probability of rejecting the null hypothesis when it is indeed false.

- **Power depends on:** effect size ($\tau$), sample size ($N$), probability of treatment ($p$), variance of potential outcomes ($\sigma$), test statistic, randomization scheme (simple, complete, clustered, blocked, etc.)
...
## [[🔙](#optimal-p)]{.small-font} Derivation of Optimal $p^*$ {#proof-optimal-p visibility="uncounted"}

<br>

- **Problem**: Minimize $\var(\hat\tau) = \frac{\sigma^2_1}{p N}+\frac{\sigma^2_0}{(1-p) N}$ over $p$.

- Find $p^*$ that makes the derivative with respect to $p$ equal to zero:
...
## [[🔙](#power-formula)]{.small-font} Derivation of the Power Function {#proof-power-formula visibility="uncounted"}

:::{.small-font}

- Suppose $\sigma^2_0=\sigma^2_1 = \sigma^2$, $p=0.5$ (so $N_0=N_1=N/2$), and $\tau=\mu_1-\mu_0$.

- The $t$-statistic under $H_a$:
...
- Generalizing to arbitrary $p$, $\sigma_1^2 \neq \sigma_0^2$: replace $2\sigma/\sqrt{N}$ with $\sqrt{\sigma_1^2/(pN) + \sigma_0^2/((1-p)N)}$.
