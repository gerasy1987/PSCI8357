...
- Unbiased for the _SATE_ under [complete randomization]{.highlight}.
...
## Unbiasedness of _IPW_ {#ipw-unbiased}

<br>

- [Inverse Probability Weighting Estimator]{.highlight} (Horvitz–Thompson estimator) is unbiased for the _SATE_ under [simple randomization]{.highlight}
...
- This estimator is general:
  ...
  1. _DiM_ is a special case of _IPW_ estimator when... [$p = N_1/N$]{.fragment}
  2. _IPW_ estimator will show up in observational studies as well.
...
1. **Difference-in-Means** (or an estimator of the _ATE_)  
...
- [Intuition]{.note}: Demeaning covariates ensures that the interaction terms $(X_i - \overline{X})' \delta$ are orthogonal to the treatment indicator $T_i$. This orthogonality helps in isolating the treatment effect $\tau$ from the covariate effects, reducing multicollinearity and improving the precision of the estimator. As the sample size increases, the bias introduced by this method diminishes, leading to a consistent estimator of the average treatment effect.
...
# Practical Concern: External Validity
...
$$
\frac{\var(\hat\tau_{CL})}{\var(\hat\tau_{CR})} = 1 + (\overline{N} - 1)\rho, \quad \text{where} \quad \bar{N} = \frac{1}{G} \sum_{j=1}^G N_j
$$

- [Intuition]{.note}: Correlation within clusters means that there is less independent information in the data than if all observations were independent. As a result, the _effective sample_ size is smaller, leading to larger standard errors.
...
- **Valid inference**:
  ...
  - Standard error estimates via **cluster-robust standard errors** (_CR2_) or randomization inference.

- [Note]{.note}: When $G$ is small, $\rho$ will be poorly estimated and cluster SEs will be unreliable $\rightsquigarrow$ prefer increasing the number of clusters over sample size per cluster.
...
:::{.column width="50%"}
- Ignoring clustering in standard error estimates can result in overly optimistic confidence intervals and increased [Type I]{.highlight} error (_false-positives_) rates.

- The simulation on the right shows the [coverage]{.highlight}--probability of 95% confidence intervals including the true effect:

  - We would expect the these CIs will include _true_ effect in 95% of the cases.
...
- [Block (stratified) randomized experiment]{.highlight}:
  ...
  - Choose $N_{1,\text{v}}$ voters to be treated, $N_{0,\text{v}} = N_{1,\text{v}} − N_{1,\text{v}}$
control.
...
- Finite sample variance of the blocked (_{BR}R_) estimator:
...
$$
Y_i = \alpha + \tau T_i + \sum_{j=2}^{M} \beta_j B_{ij} + \epsilon_i, \quad \text{where} \quad \mathbb{E}[\hat{\tau}_{OLS}] = \tau
$$
