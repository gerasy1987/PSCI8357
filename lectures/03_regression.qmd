...
get_bias_het <- function(tau_base = .5,
  ...
  tau_total <- sum(het_fun(1:100) / 100) # total treatment effect (assuming heterogeneity)
  ...
  # economic growth (influenced by both investment and democratic institutions)
  growth <-
    het_fun(X) * democracy_high +
    gamma * X + rnorm(n, mean = 0, sd = 5)

  growth_2 <-
    het_fun(X) * democracy_high_2 +
    gamma * X + rnorm(n, mean = 0, sd = 5)
...
sims_df <-
  pbapply::pbreplicate(1000, get_bias(delta = 0.5, gamma = 0.5), cl = "future") |>
  t() |>
  as.data.frame() |>
...
# Convert sims matrix to long format dataframe
sims_df <-
  pbapply::pbreplicate(1000, get_bias(delta = 0.1, gamma = 0.5), cl = "future") |>
  t() |>
  as.data.frame() |>
  rename(Biased = biased, Unbiased = unbiased) |>
  pivot_longer(cols = c(Biased, Unbiased), names_to = "Type", values_to = "Value")
...
# Convert sims matrix to long format dataframe
sims_df <-
  pbapply::pbreplicate(1000, get_bias(delta = 0.5, gamma = 0.1), cl = "future") |>
  t() |>
  as.data.frame() |>
  rename(Biased = biased, Unbiased = unbiased) |>
  pivot_longer(cols = c(Biased, Unbiased), names_to = "Type", values_to = "Value")
...
# Convert sims matrix to long format dataframe
sims_df <-
  pbapply::pbreplicate(1000, get_bias(delta = 0.1, gamma = 0.1), cl = "future") |>
  t() |>
  as.data.frame() |>
  rename(Biased = biased, Unbiased = unbiased) |>
  pivot_longer(cols = c(Biased, Unbiased), names_to = "Type", values_to = "Value")
...
- Recall [regression anatomy]{.highlight}: $\hat{\tau} = \frac{\cov(\tilde{Y}_i, \tilde{T}_i)}{\var(\tilde{T}_i)}$, where $\tilde{T}_i$ is residuals from regression of $T_i$ on other regressors
...
- Recall that $\hat{\tau} = \frac{\cov(Y_i, \tilde{T_i})}{\var(\tilde{T_i})}$, where $\tilde{T_i}$ is residuals from regression of $T_i$ on other regressors
...
$$
  \begin{align*}
  \hat{\tau} &= \frac{\E [\E [Y_i \given T_i, X_i](T_i - \E [T_i \given X_i])]}{\E [(T_i - \E [T_i \given X_i])^2]} \\
  ...
  &\class{fragment}{{}= \frac{\E [ \tau_X \textcolor{#d65d0e}{(T_i - \E [T_i \given X_i])^2} ]}{\E [(T_i - \E [T_i \given X_i])^2]}  \quad \text{($\because \pm \E [T_i \given X_i$ and CIA )}} \\
  &\class{fragment}{{}= \frac{\E_X [ \tau_X \textcolor{#d65d0e}{\E [ (T_i - \E [T_i \given X_i])^2 \given X_i]} ]}{\E_X [ \textcolor{#d65d0e}{\E [ (T_i - \E [T_i \given X_i])^2 \given X_i ]} ]}  \quad \text{($\because$ iterated $\E$ )}} \\
...
- Compare
  ...
  $$
  \hat{\tau} = \frac{\sum_X \tau_X \textcolor{#d65d0e}{\Pr (T_i = 1 \given X_i = x) (1 - \Pr (T_i = 1 \given X_i = x))} \Pr(X_i = x)}  {\sum_X \textcolor{#d65d0e}{\Pr(T_i = 1 \given X_i = x) (1 - \Pr (T_i = 1 \given X_i = x))} \Pr(X_i = x)}
  $$

. . .

- $\hat{\tau}$ aggregates via [conditional variance weighting]{.highlight} with respect to $T_i$ instead of just probability.

- If $\tau_i$ was constant _wrt_ $X_i$, the precision weighting _could_ be good from an efficiency standpoint.

- If $T_i \indep X_i$, then $\hat{\tau}$ reduces to weighting by the number of units with $X_i = x$.
...
- @aronow2016does show that for arbitrary $T_i$ and $X_i$,
  ...
  $$
  \hat{\tau} \xrightarrow{p} \frac{\E [w_i \tau_i]}{\E [w_i]}, \quad \text{where } w_i = (T_i - \E [T_i | X_i])^2,
  $$
...
- The [effective sample]{.highlight} is weighted by $\hat{w}_i = (T_i - \hat{\E}[T_i | X_i])^2$  (squared residual from regression of $T_i$ on control).
...
#| eval: true
#| output: asis
#| code-line-numbers: "1-5|7-11|13-25|27-31"
...
# economic growth (influenced by both investment and democratic institutions)
growth <-
  (tau_base + 0.01 * X) * democracy_high +
  gamma * X + rnorm(n, mean = 0, sd = 5)

growth_2 <-
  (tau_base + 0.01 * X) * democracy_high_2 +
  gamma * X + rnorm(n, mean = 0, sd = 5)
...
sims_df <-
  ...
  pivot_longer(cols = c(Heterogeneous, Homogeneous), names_to = "Assignment", values_to = "Value")
...
- Ensure model congruity with data and research context:
  - Be explicit about the assumption you need to make to make regression _causal_.
  - Use DAGs and the back-door criterion to identify covariates to control for.
  - Check for heterogeneous effects, interpreting results accordingly.
  - Use simulations or robustness checks to validate model assumptions and relationships.
