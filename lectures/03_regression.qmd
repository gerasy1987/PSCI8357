...
- [Key insight]{.note}: Colliders have special properties:
  ...
  - [Conditioning on a collider **opens** the path!]{.alert} This can create spurious associations or mask real relationship.
:::
...
- To flexibly control for $X_i$, use a [saturated regression]{.highlight} with dummies for each unique value of $X_i$ (one-way fixed effects):
...
- [Regression anatomy]{.highlight} can be written in two ways: $\widehat{\tau} = \frac{\cov(\tilde{Y}_i, \tilde{T}_i)}{\var(\tilde{T}_i)} = \frac{\cov(Y_i, \tilde{T}_i)}{\var(\tilde{T}_i)}$, where $\tilde{\cdot}_i$ is residuals from regression of $T_i$ on other regressors.
...
$$
  ...
  &\class{fragment}{{}= \frac{\E[T_i \tau_x \tilde{T}_i]}{\E[\tilde{T}_i^2]}  \quad \text{($\because$ $\E[\E[Y_i(0) \given X_i] (T_i - \E[T_i \given X_i])] = 0$)}} \\
