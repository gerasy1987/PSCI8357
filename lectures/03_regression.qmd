...
- But remember [bad controls]{.highlight} exist, e.g.
   ...
   - Block causal path by controling for post-treatment variables
...
- [Regression anatomy]{.highlight} is: $\widehat{\tau} = \frac{\cov(\tilde{Y}_i, \tilde{T}_i)}{\var(\tilde{T}_i)} = \frac{\cov(Y_i, \tilde{T_i})}{\var(\tilde{T_i})}$, where $\tilde{T}_i$ is residuals from regression of $T_i$ on other regressors.
