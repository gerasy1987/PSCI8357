...
- Observed difference in means is $\hat\tau = \E[Y_{i}(1) \given T_i = 1] - \E[Y_{i}(0) \given T_i = 0] = 1.5$.
...
- Why is $\tau_{ATT} = \hat\tau$? When would they not be equal?
...
\hat\tau &= \E[Y_i \given T_i=1]-\E[Y_i \given T_i=0] &&\\
        ...
        &= \underbrace{\E[Y_{i} (1) - Y_{i} (0) \given T_i=1]}_{\tau_{ATC}} + \underbrace{\E[Y_{i} (0) \given T_i=1]-\E[Y_{i} (0) \given T_i=0]}_{\text{Selection bias}} \quad \text{($\because \pm \E[Y_{i} (0) \given T_i=1]$)}
\end{align*}
$$
...
- The average treatment effect of $T$ on $Y$, $\tau_ATE$, can be defined as the average difference in $Y$ between two intervention regimes: 
