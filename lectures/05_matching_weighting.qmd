...
- **Result**: Under no unmeasured confounders (**CIA**), $\E[\widehat{\tau}_{IPW}] \xrightarrow{p} \tau_{ATE}$ (consistent).
  ...
  - [Important]{.note}: Would be unbiased if we knew the true propensity scores, $\pi(\mathbf{X}_i)$.
...
- [Hajek estimator]{.highlight} normalizes the denominator so the weights sum up to $1$:
...
- [Note]{.note}: This procedure can also be used for _ATT_ estimation.

## True vs Estimated PS; HT vs Hajek Estimator
    ...
    1000,
    {
      ...
      ht_ate_true <- 
        estimatr::horvitz_thompson(
          Y ~ D, condition_prs = probs)$coefficients[["D"]]

      ht_ate <- 
        estimatr::horvitz_thompson(
          Y ~ D, condition_prs = ps)$coefficients[["D"]]

      hajek_ate <- 
        estimatr::lm_robust(
          Y ~ D + X, weights = (D / ps) + ((1 - D) / (1 - ps)))$coefficients[["D"]]
...
ggplot(sims, aes(y = Estimator, x = Estimate, fill = Estimator)) +
  ...
  scale_color_manual(values = c("HT True" = "#928374", "HT" = "#cc241d", "Hajek" = "#689d6a")) +
  scale_x_continuous(
...
- **Approach 2**: [Weighted Linear Regression]{.highlight}
  ...
  - Running a regression of $Y_i$ and $T_i$ and $\mathbf{X}_i$, with weights $w_i$.
...
weight_asympt <-
  WeightIt::lm_weightit(
    main_for, data = data_small,
    weightit = ps, vcov = "asympt"
  )
