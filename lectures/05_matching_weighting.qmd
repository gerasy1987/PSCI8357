...
## What is Matching? {visibility="uncounted"}
...
\usetikzlibrary{arrows.meta, shapes.geometric, positioning, backgrounds}
...
% Background fill covering entire bounding box
\begin{pgfonlayer}{background}
  \fill[plotbg] (current bounding box.south west) rectangle (current bounding box.north east);
\end{pgfonlayer}
...
## Distance Metrics {visibility="uncounted"}
...
## Propensity Score Matching {visibility="uncounted"}
...
- [Two-step procedure to estimate causal effects]{.highlight}:
  ...
  2. Do matching (or weighting --- later in this lecture) on $\widehat{\pi}(\mathbf{X}_i)$.
...
- Matching on manually estimated $\widehat{\pi}(\mathbf{X}_i)$ as a single covariate is equivalent to letting `MatchIt` estimate the PS internally
...
## Covariate Balance {visibility="uncounted"}
...
## Matching in Practice {visibility="uncounted"}
...
- **Approach 2**: [Regression after Matching]{.highlight} [@abadie2022robust]
  ...
  - Running a regression of $Y_i$ on $T_i$ only using the matched samples. 
...
- **Approach 2**: [Robust Post-Matching Inference]{.highlight} [@abadie2022robust]
  ...
  - Cluster standard errors at the level of matches (or block bootstrap) to account for error interdependence (e.g. due to $\mathbf{X}_i$).
  - Valid for matching without replacement and also directly on covariates.
...
## Inverse Probability Weighting {visibility="uncounted"}

### Motivation for Weighting Estimator

- We now consider an alternative to matching that addresses some of its practical limitations.
...
- Weighting is actually a general (continuous) version of matching: 
  $$
  \begin{align*}
  \widehat{\tau}_{\rm match} &= \frac{1}{N_1} \sum_{i=1}^N T_i\left(Y_i - \frac{1}{ |\mathcal{M}_i| }\sum_{j \in \mathcal{M}_i}Y_{j}\right) \\
...
- Recall from [experiments]{.highlight}: the IPW estimator generalizes difference-in-means to unequal treatment probabilities:

$$
\widehat{\tau}_{IPW} = \frac{1}{N} \sum_{i=1}^N \left( \frac{T_i Y_i}{\pi_i} - \frac{(1-T_i) Y_i}{1-\pi_i} \right)
$$

. . .

- In experiments, $\pi_i = \Pr(T_i = 1 \given \mathcal{O}_N)$ is [known from the design]{.highlight}.

- In observational studies, $\pi(\mathbf{X}_i) = \Pr(T_i = 1 \given \mathbf{X}_i)$ must be [estimated]{.alert}.
...
- **Result**: Under CIA, $\widehat{\tau}_{IPW} \xrightarrow{p} \tau_{ATE}$ (consistent).

- Similar expression for _ATT_: $\ \widehat{\tau}_{IPW,T} = \frac{1}{N_1} \sum_{i=1}^N \left( T_i Y_i - \frac{\widehat{\pi}(\mathbf{X}_i)(1-T_i)Y_i}{1-\widehat{\pi}(\mathbf{X}_i)} \right)$

### Efficiency of Weighting Estimators

<br>

- [Two sources of inefficiency]{.highlight} in the IPW estimator:

. . .

  1. **Extreme weights**: When $\widehat{\pi}(\mathbf{X}_i) \approx 0$ or $\approx 1$, some units get weights $\gg 1$ and a single observation can dominate the estimate $\rightsquigarrow$ Normalize the weights using [Hájek estimator]{.highlight}.

. . .

  2. **True vs estimated PS**: Surprisingly, using $\widehat{\pi}(\mathbf{X}_i)$ is [more efficient]{.highlight} than using the true $\pi(\mathbf{X}_i)$.

     - [Intuition]{.note}: Estimated PS absorbs finite-sample imbalances that the true PS cannot correct for.
     - True PS only adjusts for _expected_ differences between groups.
     - [Important]{.alert}: Only true if the propensity score model is correctly specified!

### Hájek (Stabilized) Estimator

<br><br>

- [Hájek estimator]{.highlight} normalizes the denominator so the weights sum to $1$:
...
- [Intuition]{.note}: Prevents any single observation from dominating. Reduces variance, especially when PS values are extreme.

- [Note]{.note}: Hájek can still be slightly biased (ratio estimator), but the bias vanishes as $N \to \infty$.

### Weighted Least Squares

<br><br>

- Practically, [weighted least squares]{.highlight} gives automatic normalization:
...
- This is equivalent to the Hájek estimator -- WLS with IPW weights automatically normalizes.

- [Note]{.note}: Can also be used for _ATT_ and _ATC_ estimation with weight of $1$ given to units in treated or untreated group respectively.

### Simulation: HT vs Hájek, True vs Estimated PS
...
#| code-line-numbers: "1-4|10-17|19-29|31-35"
...
sims <-
  pbapply::pbreplicate(
    2000,
    {
      ...
      probs <- plogis(2 * X)
      D <- rbinom(N, 1, probs)
      Y <- true_tau * D - 2 * X + rnorm(N, sd = 1.5)
      ps <- predict(
        glm(D ~ X, family = binomial),
        type = "response"
      )
...
sims$Estimator <- factor(
  sims$Estimator,
  levels = c("HT (true PS)", "HT (estimated PS)", "Hájek")
)

rmse_labels <- sims |>
  group_by(Estimator) |>
  summarise(
    RMSE = sqrt(mean((Estimate - true_tau)^2, na.rm = TRUE)),
    .groups = "drop"
  )
...
ggplot(sims, aes(x = Estimate, y = Estimator, fill = Estimator)) +
  ...
  annotate(
    "text",
    x = true_tau, y = Inf,
    label = paste0("True ATE = ", true_tau),
    hjust = -0.1, vjust = 1.5,
    size = 3.5, colour = "#d79921", fontface = "bold"
  ) +
  geom_text(
    data = rmse_labels,
    aes(
...
### Weighting in Practice: Estimation
...
## Weighting in Practice {visibility="uncounted"}

### Weighting in Practice: Inference
...
- **Approach 1**: [Weighting as Pre-Processing]{.highlight}
  ...
  - This is standard, but it ignores the uncertainty from weighting.
...
- **Approach 3:** [Method of Moments]{.highlight} [@newey1994large]
  ...
  - View PS estimation and IPW estimation as one joint system of estimating equations (stacked moment conditions).

  - Standard sandwich/GMM variance formula for the joint system automatically propagates PS estimation uncertainty into $\widehat{\tau}$ SEs.
...
results <- tibble(
  ...
  Coefficient = c(
    tidy(weight_naive)$estimate[2],
    tidy(weight_boot)$estimate[2],
    tidy(weight_mest)$estimate[2]
  ),
  `Standard Error` = c(
    tidy(weight_naive)$std.error[2],
    tidy(weight_boot)$std.error[2],
    tidy(weight_mest)$std.error[2]
  )
)
...
### Problems with Weighting: Covariate Balancing
...
## Doubly Robust Estimation {visibility="uncounted"}
...
#| fig-height: 6.5
#| output-location: slide
#| code-line-numbers: "1|7-8,14-16|18-24|26-37|39-54|56-64"
...
get_estimates <-
  function(
    ...
    require(AIPW); require(SuperLearner)
    ...
    aipw_ate_ml <-
      AIPW$new(
        Y = Y, A = D,
        W = data.frame(X = X),
        Q.SL.library = "SL.lm", # outcome model
        g.SL.library = "SL.glm", # treatment model
        verbose = FALSE
      )$stratified_fit()$summary()$estimates$RD["Estimate"]
...
est_levels <- c("Naive Regression", "IPW", "AIPW by hand", "AIPW (SL)")
est_colors <- c(
  "Naive Regression" = "#928374", "IPW" = "#cc241d",
  "AIPW by hand" = "#689d6a", "AIPW (SL)" = "#458588"
)
...
p1 <-
  ...
  geom_vline(xintercept = 0.8, linetype = "dashed", linewidth = 0.8, colour = "#d79921") +
  scale_fill_manual(values = est_colors) +
  coord_cartesian(xlim = c(0.5, 1.6)) +
  labs(title = "Regression is wrong", x = "Estimate", y = NULL, fill = "Estimator")
...
p2 <-
  ...
  scale_fill_manual(values = est_colors) +
  coord_cartesian(xlim = c(0.5, 1.6)) +
  labs(title = "Weighting is wrong", x = "Estimate", y = NULL, fill = "Estimator")

p1 + p2 + plot_layout(ncol = 2, guides = "collect", axes = "collect_y") & theme(legend.position = "right")
...
- Standard choice: the [Wilcoxon signed rank test]{.highlight}.
...
- [Matching]{.highlight}
  ...
  - **Cons**: Might be inefficient because of dropping observations.
...
- In practice, we should try to be robust to the choice of estimation methods and conduct sensitivity analyses to assess issues with CIA.

# Appendix {visibility="uncounted"}
<!-- 
## [[🔙](#from-experiments-to-observational-studies)]{.small-font} Identification of $\E[Y_i(1)]$ via IPW {#proof-ipw-identification visibility="uncounted"}

:::{.small-font}
$$
\begin{align*}
  \E\!\left(\frac{T_i Y_i}{\pi(\mathbf{X}_i)} \given \mathbf{X}_i = \mathbf{x}\right)
  &= \E\!\left(\frac{Y_i}{\pi(\mathbf{X}_i)} \given \mathbf{X}_i=\mathbf{x},\, T_i=1\right)\Pr(T_i=1 \given \mathbf{X}_i=\mathbf{x}) & (\text{iterated expectations}) \\
  &= \E\!\left(\frac{Y_i}{\pi(\mathbf{X}_i)} \given \mathbf{X}_i=\mathbf{x},\, T_i=1\right) \pi(\mathbf{x}) \\
  &= \E(Y_i \given \mathbf{X}_i=\mathbf{x},\, T_i=1) \\
  &= \E\{Y_i(1) \given \mathbf{X}_i=\mathbf{x}\} & (\text{CIA})
\end{align*}
$$
:::

Averaging over $f(\mathbf{x})$ yields $\E\{Y_i(1)\}$. Analogous for $\E\{Y_i(0)\}$. $\qedknitr$ -->
